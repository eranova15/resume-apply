"""
Resume Application System
Main entry point for the resume optimization application
"""

import os
import sys
from pathlib import Path
from urllib.parse import urlparse
import re

# Add src to path
sys.path.insert(0, str(Path(__file__).parent))

from modules.resume_parser import ResumeParser
from modules.job_scraper import JobScraper
from modules.suggestion_engine import SuggestionEngine
from modules.resume_generator import ResumeGenerator
from modules.cover_letter_generator import CoverLetterGenerator
from modules.ui_handler import UIHandler


def sanitize_filename(text, max_length=20):
    """Convert text to a safe filename"""
    # Remove special characters
    safe = re.sub(r'[^\w\s-]', '', text)
    # Replace spaces with underscores
    safe = re.sub(r'[\s]+', '_', safe)
    # Truncate
    return safe[:max_length].strip('_')


def main():
    """Main application flow"""
    print("=" * 60)
    print("Welcome to Resume Application System")
    print("=" * 60)
    print()
    
    # Initialize components
    ui = UIHandler()
    resume_parser = ResumeParser()
    job_scraper = JobScraper()
    suggestion_engine = SuggestionEngine()
    resume_generator = ResumeGenerator()
    cover_letter_generator = CoverLetterGenerator()
    
    # Step 1: Get initial choice
    print("Step 1: Choose Action")
    print("-" * 60)
    initial_choice = ui.get_initial_choice()
    
    # Step 2: Get resume
    print("\nStep 2: Resume Input")
    print("-" * 60)
    resume_path = ui.get_resume_path()
    
    # Parse resume
    print("\nParsing your resume...")
    resume_data = resume_parser.parse(resume_path)
    print(f"✓ Resume parsed successfully")
    
    if initial_choice == 'improve':
        # Improve resume without job posting
        print("\n" + "=" * 60)
        print("Improving Resume")
        print("=" * 60)
        
        # Show current resume
        print("\nStep 3: Review Current Resume")
        ui.display_resume_for_editing(resume_data)
        
        # Get areas to improve
        print("\nStep 4: Specify Areas to Improve")
        edit_requests = ui.get_edit_requests()
        
        # Generate suggestions based on edit requests
        print("\nStep 5: Generating Improvement Suggestions...")
        # Create a mock job data for improvement context
        mock_job_data = {
            'title': 'General Improvement',
            'company': 'N/A',
            'location': 'N/A',
            'description': edit_requests,
            'requirements': {
                'technical_skills': [],
                'experience_years': None,
                'education': [],
                'certifications': [],
                'key_requirements': []
            }
        }
        suggestions = suggestion_engine.generate_suggestions(
            resume_data, mock_job_data, edit_requests
        )
        print(f"✓ Generated {len(suggestions)} suggestions")
        
        # Show suggestions
        print("\nStep 6: Review Suggestions")
        ui.display_suggestions(suggestions)
        
        # Apply suggestions and generate improved resume
        print("\nApplying suggestions...")
        updated_resume_data = suggestion_engine.apply_suggestions(
            resume_data, suggestions
        )
        
        # Generate improved resume PDF
        print("\nGenerating improved resume PDF...")
        output_dir = Path("output")
        output_dir.mkdir(exist_ok=True)
        
        resume_filename = "resume_improved.pdf"
        resume_path_out = output_dir / resume_filename
        resume_generator.generate(updated_resume_data, resume_path_out)
        print(f"✓ Resume saved to: {resume_path_out}")
        
        print("\n✓ Resume improvement complete!")
        
    else:  # initial_choice == 'apply'
        # Apply for jobs - original flow
        print("\nStep 3: Job Opportunities")
        print("-" * 60)
        job_urls = ui.get_job_urls()
        
        # Process each job
        for job_url in job_urls:
            print(f"\n{'=' * 60}")
            print(f"Processing job: {job_url}")
            print('=' * 60)
            
            # Step 4: Scrape job listing
            print("\nStep 4: Scraping job listing...")
            job_data = job_scraper.scrape(job_url)
            print(f"✓ Job listing scraped: {job_data.get('title', 'Unknown')}")
            
            # Step 5: Generate suggestions
            print("\nStep 5: Analyzing requirements and generating suggestions...")
            suggestions = suggestion_engine.generate_suggestions(resume_data, job_data)
            print(f"✓ Generated {len(suggestions)} suggestions")
            
            # Step 6: Show suggestions to user
            print("\nStep 6: Review suggestions")
            ui.display_suggestions(suggestions)
            
            # Step 7: Get user decision (loop for iterative improvement)
            current_resume_data = resume_data
            while True:
                decision = ui.get_user_decision()
                
                if decision == "accept":
                    # Apply suggestions
                    print("\nApplying suggestions to resume...")
                    updated_resume_data = suggestion_engine.apply_suggestions(
                        current_resume_data, suggestions
                    )
                    
                    # Generate new resume PDF
                    print("\nGenerating optimized resume PDF...")
                    output_dir = Path("output")
                    output_dir.mkdir(exist_ok=True)
                    
                    # Create safe filename from job title or URL
                    job_title = job_data.get('title', 'job')
                    safe_name = sanitize_filename(job_title, max_length=30)
                    
                    resume_filename = f"resume_{safe_name}.pdf"
                    resume_path_out = output_dir / resume_filename
                    resume_generator.generate(updated_resume_data, resume_path_out)
                    print(f"✓ Resume saved to: {resume_path_out}")
                    
                    # Generate cover letter
                    print("\nGenerating cover letter...")
                    cover_letter_filename = f"cover_letter_{safe_name}.pdf"
                    cover_letter_path = output_dir / cover_letter_filename
                    cover_letter_generator.generate(
                        updated_resume_data, job_data, cover_letter_path
                    )
                    print(f"✓ Cover letter saved to: {cover_letter_path}")
                    
                    print("\n✓ Application package complete!")
                    break
                    
                elif decision == "edit":
                    # Let user choose which resume to edit
                    resume_to_edit = ui.choose_resume_to_edit()
                    
                    if resume_to_edit == "original":
                        edit_data = resume_data
                    else:  # new
                        edit_data = suggestion_engine.apply_suggestions(
                            current_resume_data, suggestions
                        )
                    
                    # Show resume and let user mark areas
                    print("\nDisplaying resume for editing...")
                    ui.display_resume_for_editing(edit_data)
                    
                    edit_requests = ui.get_edit_requests()
                    
                    # Re-run engine with edit requests
                    print("\nRegenerating suggestions based on your feedback...")
                    suggestions = suggestion_engine.generate_suggestions(
                        resume_data, job_data, edit_requests
                    )
                    
                    # Show updated suggestions
                    ui.display_suggestions(suggestions)
                    
                elif decision == "improve":
                    # Apply suggestions and continue improving
                    print("\nApplying current suggestions to resume...")
                    current_resume_data = suggestion_engine.apply_suggestions(
                        current_resume_data, suggestions
                    )
                    print("✓ Suggestions applied")
                    
                    # Show the improved resume
                    print("\nImproved Resume Preview:")
                    ui.display_resume_for_editing(current_resume_data)
                    
                    # Get additional improvement requests
                    print("\nWhat additional improvements would you like?")
                    edit_requests = ui.get_edit_requests()
                    
                    # Generate new suggestions based on improved resume
                    print("\nGenerating additional improvement suggestions...")
                    suggestions = suggestion_engine.generate_suggestions(
                        current_resume_data, job_data, edit_requests
                    )
                    print(f"✓ Generated {len(suggestions)} new suggestions")
                    
                    # Show new suggestions
                    ui.display_suggestions(suggestions)
                    
                elif decision == "reject":
                    print("\nSkipping this job opportunity.")
                    break
    
    print("\n" + "=" * 60)
    print("Thank you for using Resume Application System!")
    print("=" * 60)


if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n\nProcess interrupted by user. Exiting...")
        sys.exit(0)
    except Exception as e:
        print(f"\n\nError: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
